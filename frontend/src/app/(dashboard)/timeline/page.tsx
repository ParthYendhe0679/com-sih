...
import { Clock, FolderOpen, ArrowRight, Loader2 } from 'lucide-react';

export default function TimelineContextualPage() {
    ...
    async function init() {
      try {
        ...
        if (saved && items.some(c => c.id === saved || c.case_number === saved)) {
          router.replace(`/cases/${saved}?tab=timeline`);
          return;
        }
        if (items.length === 1) {
          router.replace(`/cases/${items[0].case_number || items[0].id}?tab=timeline`);
          return;
        }
      } catch (err) {
        console.warn('Timeline router notice:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="py-24 text-center text-[var(--ink-tertiary)] animate-pulse">
        ...
        <p className="text-[13px]">Contextualizing case investigation timeline...</p>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in py-6">
      <div className="p-8 rounded-2xl border glass-panel space-y-3 text-center" style={{ borderColor: 'var(--border)' }}>
        ...
        <p className="text-[13px] text-[var(--ink-secondary)] max-w-lg mx-auto">
          Investigation chronology and audit trails belong to specific case records. Select an active case below to inspect its chronological event log.
        </p>
      </div>
            ...
            {cases.map((c) => (
              <button
                ...
                onClick={() => {
                  ...
                  router.push(`/cases/${c.case_number || c.id}?tab=timeline`);
                }}
