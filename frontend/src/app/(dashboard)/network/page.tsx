...
import { Network, FolderOpen, ArrowRight, Loader2 } from 'lucide-react';

export default function NetworkContextualPage() {
    ...
    async function init() {
      try {
        ...
        if (saved && items.some(c => c.id === saved || c.case_number === saved)) {
          router.replace(`/cases/${saved}?tab=network`);
          return;
        }
        if (items.length === 1) {
          router.replace(`/cases/${items[0].case_number || items[0].id}?tab=network`);
          return;
        }
      } catch (err) {
        console.warn('Network router notice:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="py-24 text-center text-[var(--ink-tertiary)] animate-pulse">
        ...
        <p className="text-[13px]">Contextualizing intelligence network...</p>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in py-6">
      <div className="p-8 rounded-2xl border glass-panel space-y-3 text-center" style={{ borderColor: 'var(--border)' }}>
        ...
        <p className="text-[13px] text-[var(--ink-secondary)] max-w-lg mx-auto">
          Criminal networks are contextual to specific investigation dossiers. Select an active case below to inspect its multi-source relational graph.
        </p>
      </div>
            ...
            {cases.map((c) => (
              <button
                ...
                onClick={() => {
                  ...
                  router.push(`/cases/${c.case_number || c.id}?tab=network`);
                }}
